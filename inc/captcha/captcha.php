...
class CzaksCaptcha {
    ...
    public function __construct(string $text, int $left, int $top, string $charset = 'abcdefghijklmnopqrstuvwxyz') {
        $len = mb_strlen($text, 'utf-8');
        $this->width = $left;
        $this->height = $top;
        $this->charset = preg_split('//u', $charset, -1, PREG_SPLIT_NO_EMPTY);
        $this->style = "";
        for ($i = 0; $i < $len; $i++) {
            $this->content[] = [
    ...
    public function mutate_sizes(): void {
        foreach ($this->content as &$v) {
            if (!isset($v['font-size'])) {
                $v['font-size'] = rand((int)($this->height / 3) - 4, (int)($this->height / 3) + 8);
            }
        }
    }
    public function mutate_positions(): void {
        foreach ($this->content as &$v) {
            $v['top'] += rand(-10, 10);
            $v['left'] += rand(-10, 10);
        }
    }
    public function mutate_transform(): void {
        $fromto = [
            '6' => '9', '9' => '6', '8' => '8', '0' => '0',
            'z' => 'z', 's' => 's', 'n' => 'u', 'u' => 'n',
            'a' => 'ɐ', 'e' => 'ə', 'p' => 'd', 'd' => 'p',
    ...
    public function randomize(?array &$a = null): void {
        if ($a === null) {
            $a = &$this->content;
        }
        shuffle($a);
    ...
    public function add_junk(): void {
        ...
        while ($count--) {
            ...
            switch (rand(0, 9)) {
                ...
                case 1:
                    $elem['top'] = rand(-90, -60); break;
                case 2:
                    $elem['left'] = rand(-70, -40); break;
                case 3:
    ...
    public function mutate_anchors(): void {
        foreach ($this->content as &$elem) {
            if (rand(0, 1)) {
                $elem['right'] = $this->width - $elem['left'] - (int)(0.5 * ($elem['font-size'] ?? 0));
                unset($elem['left']);
            }
            if (rand(0, 1)) {
                $elem['bottom'] = $this->height - $elem['top'] - (int)(1.5 * ($elem['font-size'] ?? 0));
                unset($elem['top']);
            }
        }
    }
  
    public function mutate_containers(): void {
        for ($i = 0; $i <= 80; $i++) {
            $new = [];
    ...
    public function mutate_margins(?array &$a = null): void {
        if ($a === null) {
            $a = &$this->content;
        }
        foreach ($a as &$v) {
    ...
    public function mutate_styles(?array &$a = null): void {
        if ($a === null) {
            $a = &$this->content;
        }
        foreach ($a as &$v) {
    ...
    public function to_html(?array &$a = null): void {
        $inside = true;
        if ($a === null) {
            if ($this->style) {
    ...
    public function rand_whitespace(int $r = 0): string {
        switch (rand($r, 4)) {
            case 0:
                return "";
            case 1:
    ...
    public function shuffle_assoc(array &$array): bool {
